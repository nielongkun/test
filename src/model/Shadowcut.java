...
//		 List<Array> ar2=new ArrayList<Array>();
		 ...
		 Set<Map.Entry<String,Map<Integer, Double>>> temp1Set = temp1.entrySet();
			for (Iterator<Map.Entry<String,Map<Integer, Double>>> it = temp1Set
				...
				artemp.setA(temresult);
				artemp.setB(me.getKey());
				ar1.add(artemp);
				n1++;
			}
		
				double d=tool.getDistance(cen1.getVector(), cen2.getVector());
				double sum=max1+max2;
				double para=para1*d+1/((n1+n2)/para2);
				if(sum<d){
					for(Array line:ar1){
						if((line.getA()>=(max1-para))&&(line.getA()<=max1)){
							result1.put(line.getB(), temp1.get(line.getB()));
						}
						
					}
				}else{
					for(Array line:ar1){
						if((line.getA()>=(d-max2-para))&&(line.getA()<=max1+para)){
							result1.put(line.getB(), temp1.get(line.getB()));
						}
